...
void do_syscall(Context *c) {
  ...
  switch (a[0]) {
	...
	case SYS_write:  Log("syscall:%s 1st arg:%d 2nd arg:%d 3rd arg:%d ret val:%d", syscall_name[a[0]], a[1], a[2], a[3], a[3]); if(a[1] == 1 || a[1] == 2) mywrite((char *)a[2], a[3]);  a[4] = a[3]; c->mepc += 4;  break;
